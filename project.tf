...
resource "aws_s3_bucket" "fabo1bucket24" {
  bucket = "fabo1bucket24"
...
resource "aws_s3_bucket_public_access_block" "public_storage" {
 depends_on = [aws_s3_bucket.fabo1bucket24]
 bucket = "fabo1bucket24"
...
resource "aws_cloudfront_distribution" "tera-cloufront1" {
    ...
    origin {
        domain_name = aws_s3_bucket.fabo1bucket24_regional_domain_name
